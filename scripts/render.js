...
const render = (mode) => {
    ...
    if (mode === "doing") { //doing이라면 isComplete가 false인 일들을 리턴해준다.
        for(let i=0; i<taskList.length; i++){
            if (!taskList[i].isComplete) { //아직 진행중이라면 (false)
                resultHTML += `
    ...
    else { //done이라면 isComplete가 true인 일들을 리턴해준다.
        for(let i = 0; i < taskList.length; i++) {
